...
def mock_vkeys():
    ...
    with patch("api.app.verification_keys_ready", return_value=True), patch(
        "api.app.get_verification_key_hash", return_value="test_hash_abc123"
...
class TestSanitizer:
    ...
    def test_check_injection_cypher(self):
        ...
        assert check_injection("MATCH (n) DELETE n")
        assert not check_injection("normal text")
        assert check_injection("DETACH DELETE n")

    def test_check_injection_xss(self):
        """Test XSS pattern detection."""
        from api.sanitizer import check_injection

        assert check_injection("<script>alert('xss')</script>")
        assert check_injection("javascript:void(0)")
        assert check_injection("onclick=doSomething()")
