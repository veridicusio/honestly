...
class DeepProveZKML:
    ...
    def __init__(
        ...
        self.circuit_dir = circuit_dir or Path(
            os.environ.get("ZKML_CIRCUIT_DIR", "circuits/zkml")
        )
    ...
    def setup(
        ...
        try:
            logger.info(f"Compiling ONNX model to zkML circuit...")
            ...
            compile_cmd = [
                ...
                "compile",
                "--model", str(onnx_path),
                "--output", str(self.circuit_dir / "anomaly_detector"),
                "--backend", "groth16",
            ]
    ...
    def _generate_witness(
        ...
        scaled_features = [
            [int(f * self.SCALE_FACTOR) for f in step]
