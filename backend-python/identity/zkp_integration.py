...
class AAIPZKIntegration:
    ...
    def _check_circuits(self) -> Dict[str, bool]:
        ...
        circuits = {
            "level3_inequality": self.artifacts_dir
            / "level3_inequality"
            / "Level3Inequality_final.zkey",
    ...
    def prove_reputation_threshold(
        ...
        if not self.circuits_available.get("level3_inequality"):
            return ZKProofResult(
                success=False, error="level3_inequality circuit not built. Run setup_level3.sh"
            )

        # Validate inputs
        if reputation_score < 0 or reputation_score > 100:
            return ZKProofResult(success=False, error="Reputation score must be 0-100")
        ...
        try:
            ...
            return ZKProofResult(success=False, error=str(e))
