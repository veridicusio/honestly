...
class ProofVerificationRequest(BaseModel):
    ...
    proof_type: str = Field(
        ..., pattern=r"^(age_proof|authenticity_proof|agent_reputation|agent_capability)$"
    )
...
def lookup_document_by_hash(document_hash: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        all_docs = (
            vault_storage.list_documents() if hasattr(vault_storage, "list_documents") else []
        )
...
async def ai_verify_proof(
    ...
    try:
        ...
        if request.proof_type == "age_proof":
            ...
            verified = zk_service.verify_authenticity_proof(
                request.proof_data, request.public_inputs or ""
            )
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid proof type"
            )
...
async def ai_verify_proofs_batch(
        ...
        try:
            ...
            if proof_req.proof_type == "age_proof":
                verified = zk_service.verify_age_proof(
                    proof_req.proof_data, proof_req.public_inputs or ""
                )
    ...
    return {
        ...
        "summary": {
            ...
            "avg_time_ms": (
                round((total_duration / len(request.proofs)) * 1000, 2) if request.proofs else 0
            ),
        },
        "timestamp": datetime.utcnow().isoformat(),
    }
...
async def ai_get_document_hash(
    ...
    try:
        if request.document_id:
            ...
            if not doc:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND, detail="Document with hash not found"
                )
            ...
            raise HTTPException(
                ...
                detail="document_id or document_hash required",
            )
        ...
        log_ai_interaction(
            agent_id=agent_request.agent_id, action="get_document_hash", success=False, error=str(e)
        )
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
...
async def ai_create_share_link(
    ...
    try:
        ...
        log_ai_interaction(
            agent_id=agent_request.agent_id, action="create_share_link", success=False, error=str(e)
        )
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
