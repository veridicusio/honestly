...
def get_stats() -> Dict[str, Any]:
    ...
    _evict_expired()
    hit_rate = _cache_stats['hits'] / (_cache_stats['hits'] + _cache_stats['misses']) if (_cache_stats['hits'] + _cache_stats['misses']) > 0 else 0
...
def load_vkey_from_disk(circuit: str) -> Optional[Dict[str, Any]]:
    ...
    vkey_path = Path(__file__).parent.parent / "zkp" / "artifacts" / circuit / "verification_key.json"
    hash_path = Path(__file__).parent.parent / "zkp" / "artifacts" / circuit / "verification_key.sha256"
