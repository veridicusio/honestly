...
class SecurityValidator:
    ...
    def _add_check(
        self, name: str, severity: Severity, passed: bool, message: str, remediation: str = ""
    ):
        self.checks.append(
            SecurityCheck(
                name=name,
    ...
    def _check_jwt_secret(self):
        ...
        if not secret:
            ...
            self._add_check(
                "JWT_SECRET", Severity.CRITICAL, True, "JWT_SECRET is properly configured"
            )
    ...
    def _check_cors_config(self):
        ...
        _ = os.getenv("STRICT_CORS", "false").lower() == "true"  # For future use

        if not enable_cors:
            self._add_check("CORS", Severity.MEDIUM, True, "CORS is disabled")
            return
        ...
        if "*" in allowed_origins:
            ...
            self._add_check(
                "CORS", Severity.HIGH, True, f"CORS configured: {allowed_origins[:50]}..."
            )
    ...
    def _check_hsts(self):
        ...
        if self.is_production:
            if not enable_hsts:
                ...
                self._add_check(
                    "HSTS", Severity.HIGH, True, f"HSTS enabled with max-age={hsts_max_age}"
                )
        else:
            self._add_check("HSTS", Severity.LOW, True, "HSTS check skipped (development mode)")
    ...
    def _check_debug_mode(self):
        ...
        if debug and self.is_production:
            ...
            self._add_check("Debug Mode", Severity.CRITICAL, True, "Debug mode properly configured")
    ...
    def _check_database_security(self):
        ...
        if self.is_production:
            if neo4j_uri.startswith("bolt://") and not neo4j_uri.startswith("bolt+s://"):
                ...
                self._add_check(
                    "Database TLS", Severity.HIGH, True, "Database connection appears secure"
                )
        else:
            self._add_check(
                "Database TLS", Severity.LOW, True, "Database TLS check skipped (development mode)"
            )
    ...
    def _check_secrets_exposure(self):
        ...
        if log_level == "DEBUG" and self.is_production:
            ...
            self._add_check("Log Level", Severity.MEDIUM, True, f"Log level: {log_level}")
    ...
    def _check_ssl_redirect(self):
        ...
        if self.is_production and not ssl_redirect:
            ...
            self._add_check(
                "SSL Redirect", Severity.MEDIUM, True, "SSL redirect properly configured"
            )
