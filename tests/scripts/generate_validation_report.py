...
def generate_report():
    ...
    # Overall status
    all_pass = (
        report["summary"]["load_testing"].get("status") == "PASS" and
        report["summary"]["security_testing"].get("status") == "PASS" and
        report["summary"]["chaos_testing"].get("status") == "PASS"
    )
        ...
        if "metrics" in load_data:
            ...
            if values:
                ...
                for name, value in percentiles.items():
                    ax.axvline(value, color='orange', linestyle=':', linewidth=1.5, label=f'{name}: {value:.2f}ms')
        ...
        if "metrics" in load_data:
            ...
            if values:
                ...
                for i, (name, time) in enumerate(percentiles.items()):
                    ax.text(i, time + 5, f'{time:.2f}ms', ha='center', va='bottom', fontweight='bold')
